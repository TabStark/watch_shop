import 'package:flutter/material.dart';

class AppColor {
  final Color transparent = Colors.transparent;
  final Color black = Colors.black;
  final Color black54 = Color.fromARGB(247, 24, 23, 23);
  final Color yellow = Colors.yellow;
  final Color yellow200 = Color.fromARGB(255, 243, 215, 113);
  final Color white = Colors.white;
  final Color green = Colors.green;
  final Color grey = Color.fromARGB(255, 60, 60, 60);
  final Color lightgrey = const Color.fromARGB(255, 137, 137, 137);
  final Color oriGrey = Colors.grey.shade300;
  final Color blue = Colors.blue;
  final Color navyblue = Color.fromARGB(255, 19, 45, 70);
  final Color goldBrown = Color(0xffd2a262);
  final Color darkgoldBrown = Color(0xff99702b);
  final Color darkgreen = Color.fromARGB(255, 6, 72, 6);
  final Color dkgreen = Color.fromARGB(255, 1, 30, 1);
}
